#include "Dri_NVS.h"

nvs_handle_t my_handle;

static void Dri_NVS_Open(nvs_handle_t *handle)
{
    // Open NVS
    MY_LOGI("\n");
    MY_LOGI("Opening Non-Volatile Storage (NVS) handle... ");

    esp_err_t err = nvs_open("storage", NVS_READWRITE, handle);
    if (err != ESP_OK)
    {
        MY_LOGI("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
    }
    else
    {
        MY_LOGI("Done\n");
        MY_LOGI("NVS is opened successfully");
    }
}

static void Dri_NVS_Close(nvs_handle_t handle)
{
    nvs_close(handle);
    MY_LOGI("NVS handle is closed");
}

void Dri_NVS_Init(void)
{
    // Initialize NVS
    esp_err_t err = nvs_flash_init();
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND)
    {
        // NVS partition was truncated and needs to be erased
        // Retry nvs_flash_init
        ESP_ERROR_CHECK(nvs_flash_erase());
        err = nvs_flash_init();
    }
    ESP_ERROR_CHECK(err);
    MY_LOGI("NVS is initialized successfully");
}

esp_err_t Dri_NVS_Write_I8(const char *key, int8_t value)
{
    // open nvs
    Dri_NVS_Open(&my_handle);
    // Write
    MY_LOGI("Writing NVS... ");
    esp_err_t err = nvs_set_i8(my_handle, key, value);
    if (err != ESP_OK)
    {
        MY_LOGI("Error (%s) writing NVS!\n", esp_err_to_name(err));
        return err;
    }
    // Commit written value.
    err = nvs_commit(my_handle);
    if (err != ESP_OK)
    {
        MY_LOGI("Error (%s) commiting NVS!\n", esp_err_to_name(err));
        return err;
    }
    // Close NVS
    Dri_NVS_Close(my_handle);
    return err;
}
/**
 * @brief  匹配密码正确与否，如123456密码，输入密码123456，则返回正确
 *         输入密码123 密码错误，则返回错误
 *           输入密码1123456789 ，密码正确，则返回正确
 *
 * @param  key     Comment
 *
 */
esp_err_t Dri_NVS_IsMatcheKey(const char *key)
{
    Dri_NVS_Open(&my_handle);

    /*找到所有的key,和当前传入的key是否存在,如果存在,则成功，否则失败 */
    nvs_iterator_t it = NULL;
    esp_err_t res = nvs_entry_find_in_handle(my_handle, NVS_TYPE_ANY, &it);
    while (res == ESP_OK)
    {
        nvs_entry_info_t info;
        nvs_entry_info(it, &info);
        // 寻找匹配的key
        if (strstr(key, info.key) != NULL)
        {
            MY_LOGI("找到匹配的key: %s", info.key);
            // 释放迭代器
            nvs_release_iterator(it);
            // 关闭nvs
            Dri_NVS_Close(my_handle);
            return ESP_OK;
        }
        res = nvs_entry_next(&it);
    }

    // 释放迭代器
    nvs_release_iterator(it);
    // 关闭nvs
    Dri_NVS_Close(my_handle);
    return ESP_FAIL;
}

esp_err_t Dri_NVS_DelKey(const char *key)
{

    Dri_NVS_Open(&my_handle);
    // Delete
    MY_LOGI("Deleting NVS... ");
    esp_err_t err = nvs_erase_key(my_handle, key);
    if (err != ESP_OK)
    {
        MY_LOGI("Error (%s) deleting NVS!\n", esp_err_to_name(err));
        return err;
    }

    // Commit written value.
    err = nvs_commit(my_handle);
    if (err != ESP_OK)
    {
        MY_LOGI("Error (%s) commiting NVS!\n", esp_err_to_name(err));
        return err;
    }
    // Close NVS
    Dri_NVS_Close(my_handle);
    return err;
}